<template>
  <BaseDialog :show="show" :title="'用户 API 密钥'" width="wide" @close="$emit('close')">
    <div v-if="user" class="space-y-4">
      ...
      <div v-else-if="apiKeys.length === 0" class="py-8 text-center"><p class="text-sm text-gray-500">{{ '此用户暂无 API 密钥' }}</p></div>
      <div v-else class="max-h-96 space-y-3 overflow-y-auto">
        <div v-for="key in apiKeys" :key="key.id" class="rounded-xl border border-gray-200 bg-white p-4 dark:border-dark-600 dark:bg-dark-800">
          ...
          <div class="mt-3 flex flex-wrap gap-4 text-xs text-gray-500">
            <div class="flex items-center gap-1"><span>{{ '分组' }}: {{ key.group?.name || '无' }}</span></div>
            <div class="flex items-center gap-1"><span>{{ '创建时间' }}: {{ formatDateTime(key.created_at) }}</span></div>
          </div>
        </div>
      </div>
    </div>
