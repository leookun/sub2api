<template>
  <div class="card">
    <div class="flex items-center justify-between border-b border-gray-100 px-6 py-4 dark:border-dark-700">
      <h2 class="text-lg font-semibold text-gray-900 dark:text-white">{{ '最近使用' }}</h2>
    ...
    <div class="p-6">
      ...
      <div v-else-if="data.length === 0" class="py-8">
        <EmptyState :title="'暂无使用记录'" :description="'开始使用 API 后，您的使用历史将显示在这里。'" />
      </div>
        ...
        <div v-for="log in data" :key="log.id" class="flex items-center justify-between rounded-xl bg-gray-50 p-4 transition-colors hover:bg-gray-100 dark:bg-dark-800/50 dark:hover:bg-dark-800">
          ...
          <div class="text-right">
            <p class="text-sm font-semibold">
              <span class="text-green-600 dark:text-green-400" :title="'实际'">${{ formatCost(log.actual_cost) }}</span>
              <span class="font-normal text-gray-400 dark:text-gray-500" :title="'标准'"> / ${{ formatCost(log.total_cost) }}</span>
            </p>
