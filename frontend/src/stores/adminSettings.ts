...
export const useAdminSettingsStore = defineStore("adminSettings", () => {
  ...
  const opsMonitoringEnabled = ref(
    readCachedBool("ops_monitoring_enabled_cached", true),
  );
  const opsRealtimeMonitoringEnabled = ref(
    readCachedBool("ops_realtime_monitoring_enabled_cached", true),
  );
  const opsQueryModeDefault = ref(
    readCachedString("ops_query_mode_default_cached", "auto"),
  );
  ...
  async function fetch(force = false): Promise<void> {
    ...
    try {
      ...
      writeCachedBool(
        ...
        opsMonitoringEnabled.value,
      );

      opsRealtimeMonitoringEnabled.value =
