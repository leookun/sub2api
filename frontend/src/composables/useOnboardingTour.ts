...
export function useOnboardingTour(options: OnboardingOptions) {
  ...
  const isInteractiveStep = (step: DriveStep): boolean => {
    return (
      step.popover?.showButtons?.length === 1 &&
      ...
      onNextClick: async (_el, _step, { config, state }) => {
        ...
        if (state.activeIndex === (config.steps?.length ?? 0) - 1) {
          ...
          if (
            ...
            if (targetElement) {
              const isClickable = !["INPUT", "TEXTAREA", "SELECT"].includes(
        ...
        try {
          ...
          if (
            ...
            if (!popover.description.querySelector(`.${hintClass}`)) {
              ...
              const textNode = document.createTextNode("按 Enter 或点击继续");
          ...
          if (!footerEl.classList.contains(CLASS_REORGANIZED)) {
            ...
            const progressEl = footerEl.querySelector(
              `.${CLASS_PROGRESS_TEXT}`,
            );
            const nextBtnEl =
              nextButton || footerEl.querySelector(`.${CLASS_NEXT_BTN}`);
            const prevBtnEl =
              previousButton || footerEl.querySelector(`.${CLASS_PREV_BTN}`);
            ...
            shortcut1.appendChild(document.createTextNode(` ${"翻页"}`));
            ...
            shortcut2.appendChild(document.createTextNode(` ${"退出"}`));
          ...
          // 3. 状态更新
          const isLastStep =
            state.activeIndex === (config.steps?.length ?? 0) - 1;
          const activeNextBtn =
            nextButton || footerEl.querySelector(`.${CLASS_NEXT_BTN}`);
        ...
        if (isInteractiveStep(step) && element) {
          ...
          const clickHandler = async () => {
            ...
            await new Promise((resolve) =>
              setTimeout(resolve, TIMING.INTERACTIVE_WAIT_MS),
            );
            ...
            if (nextStep?.element && typeof nextStep.element === "string") {
              const exists = await ensureElement(
                nextStep.element,
                TIMING.ELEMENT_TIMEOUT_MS,
              );
              if (!exists) {
                console.warn(
                  `Onboarding: Next step element not found: ${nextStep.element}`,
                );
                return;
              }
            }
