    ...
    <div :class="[isFullscreen ? 'p-4 md:p-6' : '', 'space-y-6 pb-12']">
      ...
      <template v-if="!isFullscreen">
        ...
        <BaseDialog
          ...
          :title="'告警规则'"
          width="extra-wide"
          @close="showAlertRulesCard = false"
        >
          <OpsAlertRulesCard />
        </BaseDialog>
...
const allowedTimeRanges = new Set<TimeRange>([
  "5m",
  "30m",
  "1h",
  "6h",
...
const applyRouteQueryToState = () => {
  ...
  groupId.value =
    typeof groupIdRaw === "number" && groupIdRaw > 0 ? groupIdRaw : null;
  ...
  if (nextMode && allowedQueryModes.has(nextMode as QueryMode)) {
    ...
    queryMode.value = allowedQueryModes.has(fallback as QueryMode)
      ? (fallback as QueryMode)
      : "auto";
  }
...
const buildQueryFromState = () => {
  ...
  if (typeof groupId.value === "number" && groupId.value > 0)
    next[QUERY_KEYS.groupId] = String(groupId.value);
...
const syncQueryToRoute = useDebounceFn(async () => {
  ...
  const sameValues =
    sameLength &&
    nextKeys.every((k) => String(curr[k] ?? "") === String(nextQuery[k] ?? ""));
...
function handleThroughputSelectGroup(nextGroupId: number) {
  const id =
    Number.isFinite(nextGroupId) && nextGroupId > 0 ? nextGroupId : null;
  groupId.value = id;
}
...
function handleOpenRequestDetails(preset?: OpsRequestDetailsPreset) {
  ...
  if (!requestDetailsPreset.value.title)
    requestDetailsPreset.value.title = basePreset.title;
...
async function refreshOverviewWithCancel(
  ...
  if (!opsEnabled.value) return;
  try {
    const data = await opsAPI.getDashboardOverview(buildApiParams(), {
      signal,
    });
...
async function refreshThroughputTrendWithCancel(
  fetchSeq: number,
  signal: AbortSignal,
) {
  if (!opsEnabled.value) return;
...
async function refreshLatencyHistogramWithCancel(
  fetchSeq: number,
  signal: AbortSignal,
) {
  if (!opsEnabled.value) return;
...
async function refreshErrorTrendWithCancel(
  fetchSeq: number,
  signal: AbortSignal,
) {
  if (!opsEnabled.value) return;
...
async function refreshErrorDistributionWithCancel(
  fetchSeq: number,
  signal: AbortSignal,
) {
  if (!opsEnabled.value) return;
  ...
  try {
    const data = await opsAPI.getErrorDistribution(buildApiParams(), {
      signal,
    });
    if (fetchSeq !== dashboardFetchSeq) return;
  ...
  try {
    await Promise.all([
      ...
      refreshThroughputTrendWithCancel(
        fetchSeq,
        dashboardFetchController.signal,
      ),
    ...
    if (autoRefreshEnabled.value) {
      autoRefreshCountdown.value = Math.floor(
