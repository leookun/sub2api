...
function validateRuntimeSettings(settings: OpsAlertRuntimeSettings): ValidationResult {
  ...
  if (silencing?.enabled) {
    ...
    if (until) {
      ...
      if (!Number.isFinite(parsed)) errors.push('静默时间必须为合法的 RFC3339 时间戳')
    }
...
<template>
  <div class="rounded-3xl bg-white p-6 shadow-sm ring-1 ring-gray-900/5 dark:bg-dark-800 dark:ring-dark-700">
    <div class="mb-4 flex items-start justify-between gap-4">
      <div>
        <h3 class="text-sm font-bold text-gray-900 dark:text-white">{{ '运维监控运行设置' }}</h3>
        <p class="mt-1 text-xs text-gray-500 dark:text-gray-400">{{ '配置存储在数据库中，无需修改 config 文件即可生效。' }}</p>
      </div>
      <button
    ...
    <div v-else class="space-y-6">
      <div class="rounded-2xl bg-gray-50 p-4 dark:bg-dark-700/50">
        <div class="mb-3 flex items-center justify-between">
          <h4 class="text-sm font-semibold text-gray-900 dark:text-white">{{ '告警评估器' }}</h4>
          <button class="btn btn-sm btn-secondary" @click="openAlertEditor">{{ '编辑' }}</button>
        </div>
  ...
  <BaseDialog :show="showAlertEditor" :title="'告警评估器'" width="extra-wide" @close="showAlertEditor = false">
    <div v-if="draftAlert" class="space-y-4">
      ...
      <div>
        <div class="mb-1 text-xs font-medium text-gray-600 dark:text-gray-300">{{ '评估间隔（秒）' }}</div>
        <input
      ...
      <div class="rounded-2xl bg-gray-50 p-4 dark:bg-dark-700/50">
        <div class="mb-2 text-sm font-semibold text-gray-900 dark:text-white">{{ '告警静默（维护模式）' }}</div>
        ...
        <div v-if="draftAlert.silencing.enabled" class="mt-4 space-y-4">
          <div>
            <div class="mb-1 text-xs font-medium text-gray-600 dark:text-gray-300">{{ '静默截止时间（RFC3339）' }}</div>
            <input
            ...
            />
            <p class="mt-1 text-xs text-gray-500 dark:text-gray-400">{{ '建议填写截止时间，避免忘记关闭静默。' }}</p>
          </div>

          <div>
            <div class="mb-1 text-xs font-medium text-gray-600 dark:text-gray-300">{{ '原因' }}</div>
            <input
          ...
          <div class="rounded-xl border border-gray-200 bg-white p-4 dark:border-dark-700 dark:bg-dark-800">
            <div class="flex items-start justify-between gap-4">
              <div>
                <div class="text-xs font-bold text-gray-900 dark:text-white">{{ '高级：定向静默' }}</div>
                <p class="text-[11px] text-gray-500 dark:text-gray-400">{{ '可选：仅静默特定规则或特定级别。字段留空表示匹配全部。' }}</p>
              </div>
            ...
            <div v-else class="mt-4 space-y-4">
              ...
              >
                <div class="mb-3 flex items-center justify-between">
                  ...
                  <button class="btn btn-sm btn-danger" type="button" @click="removeSilenceEntry(idx)">{{ '删除' }}</button>
                </div>

                <div class="grid grid-cols-1 gap-3 md:grid-cols-2">
                  <div>
                    <div class="mb-1 text-xs font-medium text-gray-600 dark:text-gray-300">{{ '规则ID（可选）' }}</div>
                    <input
                  ...
                  <div>
                    <div class="mb-1 text-xs font-medium text-gray-600 dark:text-gray-300">{{ '级别（可选）' }}</div>
                    <input
                  ...
                  <div>
                    <div class="mb-1 text-xs font-medium text-gray-600 dark:text-gray-300">{{ '截止时间（RFC3339）' }}</div>
                    <input
      ...
      <details class="rounded-lg border border-gray-200 bg-gray-50 p-3 dark:border-dark-600 dark:bg-dark-800">
        ...
        <div class="mt-3 grid grid-cols-1 gap-4 md:grid-cols-2">
          ...
          <div class="md:col-span-2">
            <div class="mb-1 text-xs font-medium text-gray-500">{{ '分布式锁 Key' }}</div>
          ...
          <div>
            <div class="mb-1 text-xs font-medium text-gray-500">{{ '分布式锁 TTL（秒）' }}</div>
    ...
    <template #footer>
      <div class="flex justify-end gap-2">
        <button class="btn btn-secondary" @click="showAlertEditor = false">{{ '取消' }}</button>
