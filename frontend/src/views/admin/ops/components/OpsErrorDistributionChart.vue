...
const categories = computed<ErrorCategory[]>(() => {
  ...
  if (upstream > 0)
    out.push({ label: "上游", count: upstream, color: colors.value.orange });
  if (client > 0)
    out.push({ label: "客户端", count: client, color: colors.value.blue });
  if (system > 0)
    out.push({ label: "系统", count: system, color: colors.value.red });
  if (other > 0)
    out.push({ label: "其他", count: other, color: colors.value.gray });
  return out;
});
