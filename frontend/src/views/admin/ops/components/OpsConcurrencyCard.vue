...
const realtimeEnabled = computed(() => {
  return (
    (concurrency.value?.enabled ?? true) &&
    (availability.value?.enabled ?? true)
  );
});
...
const platformRows = computed((): SummaryRow[] => {
  ...
  const platforms = new Set([
    ...Object.keys(concStats),
    ...Object.keys(availStats),
  ]);
    ...
    .map((platform) => {
      ...
      return {
        ...
        availability_percentage:
          totalAccounts > 0
            ? Math.round((availableAccounts / totalAccounts) * 100)
            : 0,
        concurrency_percentage:
          totalConcurrency > 0
            ? Math.round((usedConcurrency / totalConcurrency) * 100)
            : 0,
      };
    })
...
const groupRows = computed((): SummaryRow[] => {
  ...
  const rows = Array.from(groupIds)
    .map((gid) => {
      ...
      if (
        ...
        conc.platform !== props.platformFilter &&
        avail.platform !== props.platformFilter
      ) {
        return null;
      }
  ...
  return rows.sort(
    (a, b) => b.concurrency_percentage - a.concurrency_percentage,
  );
});
...
const accountRows = computed((): AccountRow[] => {
  ...
  const rows = Array.from(accountIds)
    .map((aid) => {
      ...
      if (typeof props.groupIdFilter === "number" && props.groupIdFilter > 0) {
        if (
          conc.group_id !== props.groupIdFilter &&
          avail.group_id !== props.groupIdFilter
        ) {
          return null;
        }
      }
      ...
      return {
        key: aid,
        name: String(
          conc.account_name || avail.account_name || `Account ${aid}`,
        ),
        platform: String(conc.platform || avail.platform || ""),
...
<template>
  <div
    class="flex h-full flex-col rounded-3xl bg-white p-6 shadow-sm ring-1 ring-gray-900/5 dark:bg-dark-800 dark:ring-dark-700"
  >
    <!-- 头部 -->
    ...
    >
      <!-- 维度标题栏 -->
      <div
        class="flex shrink-0 items-center justify-between border-b border-gray-200 bg-gray-50 px-3 py-2 dark:border-dark-700 dark:bg-dark-900"
      >
        <span
      ...
      <!-- 汇总视图（平台/分组） -->
      <div
        v-else-if="displayDimension !== 'account'"
        class="custom-scrollbar max-h-[360px] flex-1 space-y-2 overflow-y-auto p-3"
      >
        <div
          v-for="row in displayRows as SummaryRow[]"
          :key="row.key"
        ...
        >
          ...
          <div class="mb-2 flex items-center justify-between gap-2">
            <div class="flex items-center gap-2">
              ...
              <span
                v-if="displayDimension === 'group' && row.platform"
            ...
            <div class="flex shrink-0 items-center gap-2 text-[10px]">
              ...
              <span
                :class="[
                  'font-bold',
                  getLoadTextClass(row.concurrency_percentage),
                ]"
              >
          ...
          <!-- 进度条 -->
          <div
            class="mb-2 h-1.5 w-full overflow-hidden rounded-full bg-gray-200 dark:bg-dark-700"
          >
            <div
          ...
          <div class="flex flex-wrap items-center gap-x-3 gap-y-1 text-[10px]">
            ...
            <div class="flex items-center gap-1">
              ...
              <span class="text-gray-400 dark:text-gray-500"
                >{{ row.availability_percentage }}%</span
              >
            </div>
      ...
      >
        <div
          v-for="row in displayRows as AccountRow[]"
          :key="row.key"
        ...
        >
          ...
          <div class="mb-1.5 flex items-center justify-between gap-2">
            ...
            <div class="flex shrink-0 items-center gap-2">
              <!-- 并发使用 -->
              <span
                class="font-mono text-[11px] font-bold text-gray-900 dark:text-white"
              >
          ...
          >
            <div
              class="h-full rounded-full transition-all duration-300"
              :class="getLoadBarClass(row.load_percentage)"
          ...
          <div v-if="row.waiting_in_queue > 0" class="mt-1.5 flex justify-end">
            <span
              class="rounded-full bg-purple-100 px-1.5 py-0.5 text-[10px] font-semibold text-purple-700 dark:bg-purple-900/30 dark:text-purple-400"
            >
              {{ `队列 ${row.waiting_in_queue}` }}
            </span>
          </div>
