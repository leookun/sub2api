...
const availableRealtimeWindows = computed(() => {
  ...
  return (["1min", "5min", "30min", "1h"] as const).filter(
    (w) => REALTIME_WINDOW_MINUTES[w] <= toolbarMinutes,
  );
});
...
function formatCustomTimeRangeLabel(
  startTime: string,
  endTime: string,
): string {
...
function getRequestErrorRateThresholdLevel(
...
function getUpstreamErrorRateThresholdLevel(
...
const totalRequestsLabel = computed(() =>
  formatNumber(overview.value?.request_count_total ?? 0),
);
const totalTokensLabel = computed(() =>
  formatNumber(overview.value?.token_consumed ?? 0),
);
...
const dbConnOpenValue = computed<number | null>(() => {
  if (dbConnActiveValue.value == null || dbConnIdleValue.value == null)
    return null;
...
const dbUsagePercent = computed<number | null>(() => {
  if (
    dbConnOpenValue.value == null ||
    dbMaxOpenConnsValue.value == null ||
    dbMaxOpenConnsValue.value <= 0
  )
    return null;
  return Math.min(
    100,
    Math.max(0, (dbConnOpenValue.value / dbMaxOpenConnsValue.value) * 100),
  );
});
...
const redisConnActiveValue = computed<number | null>(() => {
  if (redisConnTotalValue.value == null || redisConnIdleValue.value == null)
    return null;
...
const redisUsagePercent = computed<number | null>(() => {
  if (
    redisConnTotalValue.value == null ||
  ...
  return Math.min(
    100,
    Math.max(0, (redisConnTotalValue.value / redisPoolSizeValue.value) * 100),
  );
});
...
const redisMiddleLabel = computed(() => {
  ...
  if (redisUsagePercent.value != null)
    return `${redisUsagePercent.value.toFixed(0)}%`;
  if (systemMetrics.value?.redis_ok === true) return "正常";
  return "暂无数据";
});

const redisMiddleClass = computed(() => {
  if (systemMetrics.value?.redis_ok === false)
    return "text-rose-600 dark:text-rose-400";
  if (redisUsagePercent.value != null) {
    if (redisUsagePercent.value >= 90)
      return "text-rose-600 dark:text-rose-400";
    if (redisUsagePercent.value >= 70)
      return "text-yellow-600 dark:text-yellow-400";
    return "text-emerald-600 dark:text-emerald-400";
  }
  if (systemMetrics.value?.redis_ok === true)
    return "text-emerald-600 dark:text-emerald-400";
  return "text-gray-900 dark:text-white";
});
...
const jobsStatus = computed<"ok" | "warn" | "unknown">(() => {
  ...
  for (const hb of list) {
    ...
    if (
      hb.last_error_at &&
      (!hb.last_success_at || hb.last_error_at > hb.last_success_at)
    )
      return "warn";
  }
  return "ok";
...
<template>
  ...
  >
    <!-- Top Toolbar -->
    <div
      class="flex flex-wrap items-center justify-between gap-4 border-b border-gray-100 pb-4 dark:border-dark-700"
    >
      <div>
        <h1
          class="flex items-center gap-2 text-xl font-black text-gray-900 dark:text-white"
        >
          <svg
        ...
        >
          ...
          <template
            v-if="
              props.autoRefreshEnabled &&
              props.autoRefreshCountdown !== undefined
            "
          >
            <span>·</span>
    ...
    <div v-if="overview" class="grid grid-cols-1 gap-6 lg:grid-cols-12">
      ...
      >
        <div
          class="grid h-full grid-cols-1 gap-6 md:grid-cols-[200px_1fr] md:items-center"
        >
          <!-- 1) Health Score -->
          <div
          ...
          >
            ...
            >
              <div
                class="rounded-xl bg-white p-4 shadow-xl ring-1 ring-black/5 dark:bg-gray-800 dark:ring-white/10"
              >
                <h4
                ...
                <div class="space-y-3">
                  <div
                    v-for="(item, idx) in diagnosisReport"
                    :key="idx"
                    class="flex gap-3"
                  >
            ...
            <div class="relative flex items-center justify-center">
              ...
              <div class="absolute flex flex-col items-center">
                <span
                  :class="[
                    props.fullscreen ? 'text-5xl' : 'text-3xl',
                    'font-black',
                    healthScoreClass,
                  ]"
                >
          ...
          <div class="flex h-full flex-col justify-center py-2">
            <div class="mb-3 flex flex-wrap items-center justify-between gap-2">
              <div class="flex items-center gap-2">
                ...
                <HelpTooltip
                  v-if="!props.fullscreen"
                  :content="'每秒查询数（QPS）和每秒Token数（TPS），实时显示系统吞吐量。'"
                />
              </div>
              ...
              <div>
                ...
                <div class="mt-1 flex flex-wrap items-baseline gap-x-4 gap-y-2">
                  <div class="flex items-baseline gap-1.5">
                    <span
                      :class="[
                        props.fullscreen ? 'text-4xl' : 'text-xl sm:text-2xl',
                        'font-black text-gray-900 dark:text-white',
                      ]"
                      >{{ displayRealTimeQps.toFixed(1) }}</span
                    >
                    <span
                      :class="[
                  ...
                  <div class="flex items-baseline gap-1.5">
                    <span
                      ...
                      >{{ displayRealTimeTps.toFixed(1) }}</span
                    >
                    <span
                      :class="[
                        props.fullscreen ? 'text-sm' : 'text-xs',
      ...
      <div
        class="grid h-full grid-cols-1 content-center gap-4 sm:grid-cols-2 lg:col-span-7 lg:grid-cols-3"
      >
        <!-- Card 1: Requests -->
        <div
        ...
        >
          <div class="flex items-center justify-between">
            <div class="flex items-center gap-2">
              ...
              <HelpTooltip
                v-if="!props.fullscreen"
                :content="'服务等级协议达成率，排除业务限制（如余额不足、配额超限）的成功请求占比。'"
              />
              <span
                class="h-1.5 w-1.5 rounded-full"
                :class="
                  getSLAThresholdLevel(slaPercent) === 'critical'
                    ? 'bg-red-500'
                    : getSLAThresholdLevel(slaPercent) === 'warning'
                      ? 'bg-yellow-500'
                      : 'bg-green-500'
                "
              ></span>
          ...
          <div
            ...
            :class="getThresholdColorClass(getSLAThresholdLevel(slaPercent))"
          >
            {{ slaPercent == null ? "-" : `${slaPercent.toFixed(3)}%` }}
          </div>
          <div
            class="mt-3 h-2 w-full overflow-hidden rounded-full bg-gray-200 dark:bg-dark-700"
          >
            <div
        ...
        >
          <div class="flex items-center justify-between">
            <div class="flex items-center gap-1">
              ...
              <HelpTooltip
                v-if="!props.fullscreen"
                :content="'请求时长统计，包括 p50、p90、p95、p99 等百分位数。'"
              />
            </div>
            <button
              v-if="!props.fullscreen"
          ...
          <div
            class="mt-3 grid grid-cols-1 gap-x-3 gap-y-1 text-xs 2xl:grid-cols-2"
          >
        ...
        >
          <div class="flex items-center justify-between">
            <div class="flex items-center gap-1">
              ...
              <HelpTooltip
                v-if="!props.fullscreen"
                :content="'首 Token 延迟（Time To First Token），衡量流式响应的首 Token 返回速度。'"
              />
            </div>
            <button
              v-if="!props.fullscreen"
          ...
          >
            <div class="flex items-baseline gap-1 whitespace-nowrap">
              ...
              <span
                class="font-bold"
                :class="
                  getThresholdColorClass(getTTFTThresholdLevel(ttftP95Ms))
                "
                >{{ ttftP95Ms ?? "-" }}</span
              >
              <span class="text-gray-400">ms</span>
            ...
            <div class="flex items-baseline gap-1 whitespace-nowrap">
              ...
              <span
                class="font-bold"
                :class="
                  getThresholdColorClass(getTTFTThresholdLevel(ttftP90Ms))
                "
                >{{ ttftP90Ms ?? "-" }}</span
              >
              <span class="text-gray-400">ms</span>
            ...
            <div class="flex items-baseline gap-1 whitespace-nowrap">
              ...
              <span
                class="font-bold"
                :class="
                  getThresholdColorClass(getTTFTThresholdLevel(ttftP50Ms))
                "
                >{{ ttftP50Ms ?? "-" }}</span
              >
              <span class="text-gray-400">ms</span>
            ...
            <div class="flex items-baseline gap-1 whitespace-nowrap">
              ...
              <span
                class="font-bold"
                :class="
                  getThresholdColorClass(getTTFTThresholdLevel(ttftAvgMs))
                "
                >{{ ttftAvgMs ?? "-" }}</span
              >
              <span class="text-gray-400">ms</span>
            ...
            <div class="flex items-baseline gap-1 whitespace-nowrap">
              ...
              <span
                class="font-bold"
                :class="
                  getThresholdColorClass(getTTFTThresholdLevel(ttftMaxMs))
                "
                >{{ ttftMaxMs ?? "-" }}</span
              >
              <span class="text-gray-400">ms</span>
          ...
          <div
            ...
            :class="
              getThresholdColorClass(
                getRequestErrorRateThresholdLevel(errorRatePercent),
              )
            "
          >
            {{
        ...
        >
          <div class="flex items-center justify-between">
            <div class="flex items-center gap-1">
              ...
              <HelpTooltip
                v-if="!props.fullscreen"
                :content="'上游服务返回的错误，包括API提供商的错误响应（排除429/529限流错误）。'"
              />
            </div>
            <button
              v-if="!props.fullscreen"
          ...
          <div
            ...
            :class="
              getThresholdColorClass(
                getUpstreamErrorRateThresholdLevel(upstreamErrorRatePercent),
              )
            "
          >
            {{
              upstreamErrorRatePercent == null
                ? "-"
                : `${upstreamErrorRatePercent.toFixed(2)}%`
            }}
          </div>
          <div class="mt-3 space-y-1 text-xs">
            <div class="flex justify-between">
            ...
            <div class="flex justify-between">
              ...
              <span class="font-bold text-gray-900 dark:text-white">{{
                formatNumber(
                  (overview.upstream_429_count ?? 0) +
                    (overview.upstream_529_count ?? 0),
                )
              }}</span>
            </div>
          </div>
    ...
    >
      <div class="grid grid-cols-2 gap-3 sm:grid-cols-3 lg:grid-cols-6">
        ...
        <div class="rounded-xl bg-gray-50 p-3 dark:bg-dark-900">
          ...
          >
            {{
              systemMetrics?.memory_used_mb == null ||
              systemMetrics?.memory_total_mb == null
                ? "-"
        ...
        <div class="rounded-xl bg-gray-50 p-3 dark:bg-dark-900">
          <div class="flex items-center gap-1">
            ...
            <HelpTooltip
              v-if="!props.fullscreen"
              :content="'数据库连接池状态，包括活跃连接、空闲连接和等待连接数。'"
            />
          </div>
        ...
        <div class="rounded-xl bg-gray-50 p-3 dark:bg-dark-900">
          <div class="flex items-center gap-1">
            ...
            <HelpTooltip
              v-if="!props.fullscreen"
              :content="'Redis 连接池状态，显示活跃和空闲的连接数。'"
            />
          </div>
        ...
        <div class="rounded-xl bg-gray-50 p-3 dark:bg-dark-900">
          <div class="flex items-center gap-1">
            ...
            <HelpTooltip
              v-if="!props.fullscreen"
              :content="'Go 运行时的协程数量（轻量级线程）。没有绝对安全值，建议以历史基线为准。经验参考：<2000 常见；2000-8000 需关注；>8000 且伴随队列上升时，优先排查阻塞/泄漏。'"
            />
          </div>
        ...
        <div class="rounded-xl bg-gray-50 p-3 dark:bg-dark-900">
          <div class="flex items-center justify-between gap-2">
            <div class="flex items-center gap-1">
              ...
              <HelpTooltip
                v-if="!props.fullscreen"
                :content="'后台任务执行状态，包括最近运行时间、成功时间和错误信息。'"
              />
            </div>
            <button
              v-if="!props.fullscreen"
      ...
      <div v-else class="space-y-3">
        ...
        >
          <div class="flex items-center justify-between gap-3">
            ...
            >
              <span v-if="hb.last_duration_ms != null" class="font-mono"
                >{{ hb.last_duration_ms }}ms</span
              >
